if __name__ == "__main__":
    ...
    def process_train(arguments):
        osex.set_process_lowest_prio()
        # 设置默认的GPU索引为0,1
        # if arguments.force_gpu_idxs is None:
        #     arguments.force_gpu_idxs = "0,1"
        #
        # # 设置默认的模型名为"new"
        # if arguments.force_model_name is None:
        #     arguments.force_model_name = "new"
        #
        # # 启用静默启动
        # arguments.silent_start = True
    ...
    p.add_argument('--force-model-name', dest="force_model_name", default="new", help="Forcing to choose model name from model/ folder.")
    p.add_argument('--cpu-only', action="store_true", dest="cpu_only", default=False, help="Train on CPU.")
    p.add_argument('--force-gpu-idxs', dest="force_gpu_idxs", default=None, help="Force to choose GPU indexes separated by comma.")
    p.add_argument('--silent-start', action="store_true", dest="silent_start", default=True, help="Silent start. Automatically chooses Best GPU and last used model.")
