...
class PersonalizedVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            sks_name2_local = '<Xo>'
            sks_name3_local = '<Ja>'
            sks_and_tokens = sks_name_local + 'is' + ''.join(
                [f"<Cl_token{i}>" for i in range(16)]) + " "
            sks_and_tokens2 = sks_name2_local + 'is' + ''.join(
                [f"<Xo_token{i}>" for i in range(16)]) + " "
            sks_and_tokens3 = sks_name3_local + 'is' + ''.join(
        ...
        if labels is not None:
            ...
            return_dict['sks_present3'] = video_data.get('sks_present3', None)
        return return_dict
...
def collate_fn(batch):
    ...
    if 'labels' in batch[0]:
        ...
        batch_dict['sks_present3'] = [item['sks_present3'] for item in batch]
...
def get_args():
    ...
    parser.add_argument("--model_path", type=str,
                        default="/root/autodl-tmp/yufei/InternVideo/InternVideo2/multi_modality/Internvideo2_chat_8B_HD_finetune_REMOH_3people")
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}') as pbar:
        for step, batch in enumerate(train_loader):
            ...
            writer.add_scalar('Loss/token2-norm', model.get_input_embeddings().weight[special_token_ids_2].norm().item(),
                              global_step)
            writer.add_scalar('Loss/token3-norm',
                              model.get_input_embeddings().weight[special_token_ids_3].norm().item(),
                              global_step)
...
def train(model, config, tokenizer, sks_tokens, sks_tokens2,sks_token3,prefix_tokens,prefix_tokens2,prefix_token3):
    ...
    trainable_params = [
      ...
      #  {'params': model.get_input_embeddings().weight[sks_token_ids], 'lr': config.token_lr},
        ...
        {
            'params': model.get_input_embeddings().weight[sks_token_ids_3],
            'lr': config.token_lr,
            'sks_present': config.sks_name3
        }
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % args.save_epochs == 0:
            ...
            extra_info = {
                ...
                'sks_tokens3': sks_tokens3,
                'prefix_tokens': prefix_tokens,
                'prefix_tokens2': prefix_tokens2,
                'prefix_tokens3': prefix_tokens3,
...
def test(model, tokenizer, test_path, device):
    ...
    output_path = os.path.join(os.path.dirname(test_path), f"test_results_REMOH{output['model_name']}3people.json")
