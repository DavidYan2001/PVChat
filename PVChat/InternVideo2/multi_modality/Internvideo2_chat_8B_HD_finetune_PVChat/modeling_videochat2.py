...
class InternVideo2_VideoChat2(BaseMLLM):
    ...
    def __init__(self, config, tokenizer=None):
        ...
        self.tokenizer = tokenizer  #Save the tokenizer as an attribute of the class
        self.config = config
    ...
    def ensure_complete_config(self):
        """
        Check and supplement the integrity of self.config by re-reading the original JSON configuration.
        """
        # Suppose 'name or path' is the configuration path
        ...
        try:
            ...
            raise ValueError(f"The configuration file {config_path} does not exist and cannot be supplemented.")
        ...
        for key, value in reference_config.items():
            if not hasattr(self.config, key) or getattr(self.config, key, None) is None:
                print(f"Supplementary Fields: {key} -> {value}")
    ...
    def pad_text_embeds(
        ...
        # Obtain the embeddings of all tokens
        text_embeds = self.lm.get_input_embeddings()(input_ids.long())

        # Find the positions of the tokens that need to be kept fixed
        ...
        # Only detach the embedding of the fixed token
        ...
        if image is not None:
            ...
            prompt_video_embeds, lb_loss = self.encode_vision(video, instruction=instruction)  #Obtaining video features and text after interaction [6, 96, 768]
            visual = prompt_video_embeds  # [12, 112, 768]
            prompt_video_embeds = self.project_up(prompt_video_embeds)  # Project to the required dimension [12, 112, 4096]
            prompt_video_embeds = prompt_video_embeds.view(B, N, prompt_video_embeds.shape[-2],
                                                           prompt_video_embeds.shape[-1])  # Restore to the batch dimension [4, 336, 4096]


            visual_idx = video_idx

            video_idx_flat = video_idx.view(-1)  #
            num_video_tokens = video_idx_flat.sum().item()  # Count the total number of marks

            # flatten prompt_video_embeds 为 [num_video_tokens, hidden_dim]
            prompt_video_embeds_flat = prompt_video_embeds.view(-1, prompt_video_embeds.shape[-1])
            # Replace the position where video idx is 1 in text embeds

            text_embeds = text_embeds.view(-1, text_embeds.shape[-1])  # flatten text_embeds [2, 407, 4096]
            text_embeds[video_idx_flat == 1] = prompt_video_embeds_flat.to(text_embeds.device).to(text_embeds.dtype)
            text_embeds = text_embeds.view(video_idx.shape[0], video_idx.shape[1], -1)  # Restore the batch dimension
        else:
        ...
        return text_embeds, lb_loss  # Real text token embedding + video features
    ...
    def encode_vision(
        ...
        ## shape change: [batch_size, num_query_token, vision_width]
        if instruction is not None:
            ...
            # 2.use Qformer process visual feature
    ...
    def chat(
        ...
        if media_type == 'image':  # Obtain the number of images
            ...
            ilen = media_tensor.shape[1]  # Get the number of video frames
        ...
        for q, a in chat_history:
            conversation += (" [INST] " + q + " [/INST]")  # add history question
