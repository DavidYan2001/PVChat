...
def load_cekebv_data(train_json, test_json):
    """
    Load train.json + test.json and merge, return a list:
    [
      {
        "video_name": "...",
...
def split_workload(total_items, num_gpus):
    """Split workload according to available GPU count, return [(start1, end1), (start2, end2), ...]"""
...
def get_available_gpus():
    """Detect available GPUs in the system, return ID list"""
    import subprocess
    try:
        ...
        for line in nvidia_smi.stdout.strip().split('\n'):
            ...
            if total == 0:
                continue
            # If GPU memory usage is less than 20%, consider it available
            if (used / total) < 0.2:
...
def normalize_age(age_str):
    """Convert "middle-aged" -> "middle_aged" etc."""
    mapping = {
        "child": "child",
        "young": "young",
...
def get_prompts_for_gender_age(class_data, gender, age, num_prompts=10):
    """
    Find corresponding subtree in class_data based on gender + age, collect all leaf descriptions, and randomly select num_prompts items.
    """
...
def generate_videos_for_person(
    ...
    for idx_p, prompt in enumerate(prompts, start=1):
        ...
        cmd = [
            "python",
            consisid_script_path,  # Global variable passed from outside
...
def generate_videos_for_range(
    ...
    """
    Process videos in range video_info_list[start_idx:end_idx] on specified GPU.
    """
    # Set CUDA device
    ...
    for i in range(start_idx, end_idx):
        ...
        # If not both, clear second person's prompts
        if sample_type != "both":
            prompts_2 = []

        # =============== New: Separate two output directories ===============
        # Single person or first person => consisid_generation
        consisid_dir_1 = os.path.join(video_output_dir, "consisid_generation")
        os.makedirs(consisid_dir_1, exist_ok=True)

        # Second person => consisid_generation2
        consisid_dir_2 = os.path.join(video_output_dir, "consisid_generation2")
        os.makedirs(consisid_dir_2, exist_ok=True)

        # =============== Skip Check ===============
        # For person1 => If consisid_dir_1 already has >= 10 .mp4 files, skip
        # For person2 => If consisid_dir_2 already has >= 10 .mp4 files, skip
        # In single person scenario, person2 won't execute
        ...
        # ============ Generate second person (only if sample_type == 'both') ============
        if sample_type == "both":
            if not skip_person2:
...
def run_parallel_processing(
    ...
    """
    Main entry function:
    1) Read train.json + test.json and merge
    2) Read class_data_modified.json
    3) Randomly shuffle video list
    4) Multi-process processing
    """
    global consisid_script_path
    consisid_script_path = consisid_script  # Global variable for sub-functions
    ...
    # Randomly shuffle list for more even distribution across multiple GPUs
    random.shuffle(video_info_list)

    # Assign work according to available GPUs
